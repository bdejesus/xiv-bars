...
export default class PlayerActions {
  ...
  static jsonToQuery(json, separator='&') {
  ...
  async decorateActions(
    ...
    const upgradeActionsFile = `${src}/UpgradableActions.json`;
    let upgradeableActions = {};

    fs.readFile(upgradeActionsFile, 'utf8', (err, data) => {
      upgradeableActions = data ? JSON.parse(data) : {};
    })
    ...
    return actionsJson
      ...
      .map((action) => ({
        ...
        Command,
        IsUpgradable: upgradeableActions[this.job.Abbreviation]?.includes(action.fields.Name) || false
      }))
      .sort(byKey('ID'));
  }
  ...
  async fetchActions({ sheets, query }) {
    ...
    const queryString = `(${PlayerActions.jsonToQuery(baseQuery, ' ')})`;

    const params = PlayerActions.jsonToQuery({
      sheets,
      query: queryString,
      limit,
      ...defaultFields,
