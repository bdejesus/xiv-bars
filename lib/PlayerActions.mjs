...
import UpgradableActions from '../.apiData/UpgradableActions.json' assert { type: 'json' };
  ...
  decorateActions(
    ...
    return actionsJson
      ...
      .map((action) => ({
        ...
        Command,
        IsUpgradable: UpgradableActions[this.job.Abbreviation]?.includes(action.fields.Name)
      }))
      .sort(byKey('ID'));
  }
