...
export default class PlayerActions {
  ...
  async #decorateActions(
    ...
    return actionsJson
      ...
      .map((action) => {
        ...
        const Description = action.transient?.Description;

        let ComboActions = [];
        if (Description?.match('Combo Action:')) {
          ComboActions = Description
            .split('Combo Action: ')[1]
            .split('\n')[0]
            .split(' or ');
        }
        ...
        return {
          ...
          Level,
          UpgradeActions: upgradableActions,
          ComboActions
        }
      })
      .sort(byKey('ID'));
  ...
  async #UpgradableActions() {
    ...
    const rows = traits
      ...
      .filter((row) => (
        row?.match(/^Upgrades/)
        && !row.match(/^Upgrades.*when|.*executed by|.*while under|.*is upgraded/)
        && !row.match(/increases the | the potency of/)
      ))
      // Parse the text to only take Action Names that are upgradable
      .map((text) => {
        if (text.match(/respectively/)) {
          const actionString = text
            .split(' to ')[0]
            .replace('Upgrades ', '')
            .replace('.', '')
            .split(' and ');
