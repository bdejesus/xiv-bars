...
async function getActions(filters) {
  const query = `(${['IsPlayerAction=1', ...filters].join(' ')})`;
...
export async function listJobActions(job, pvp) {
  ...
  const jobParams = [`ClassJob.Abbreviation="${job.Abbr}"`];
  if (['DOM', 'DOW'].includes(job.Discipline)) jobParams.push(`IsPvP=${isPvP}`);

  const actionsReq = await getActions(jobParams);
  ...
  if (job.ClassID !== null) {
    ...
    const classJobParams = [`ClassJob.Abbreviation="${classJob.Abbr}"`];
    if (['DOM', 'DOW'].includes(job.Discipline)) classJobParams.push(`IsPvP=${isPvP}`);
    const classActionRequest = await getActions(classJobParams);
